...
from crud import (
    ...
    create_notification,
    create_seed_user,
    ensure_purchase_orders_for_approved_prs,
)
...
def seed_database():
    ...
    try:
        ...
        updated = False
        supplier_defaults = {
            "PR-2026-0001": "Dell Australia",
            "PR-2026-0002": "Officeworks",
            "PR-2026-0003": "Harvey Norman",
        }
        for pr in db.query(PurchaseRequisition).all():
            if not pr.supplier_name or pr.supplier_name == "Unknown Supplier":
                pr.supplier_name = supplier_defaults.get(pr.pr_number, "Officeworks")
                updated = True
        if updated:
            db.commit()

        ensure_purchase_orders_for_approved_prs(db)
    finally:
        db.close()
